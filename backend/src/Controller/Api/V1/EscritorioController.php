...
use App\Service\Escritorio\EscritorioService;
...
#[Route('/api/v1/escritorios')] #[OA\Tag(name: 'Escritórios regionais')]
final readonly class EscritorioController
{
    public function __construct(private EscritorioService $service, private ValidatorInterface $validator) {}
    ...
    private function dto(Request $request): SalvarEscritorioDto { try { $dados = json_decode($request->getContent(), true, 512, JSON_THROW_ON_ERROR); } catch (\JsonException) { throw new BadRequestHttpException('JSON inválido.'); } $dto = SalvarEscritorioDto::deArray(is_array($dados) ? $dados : []); $erros = $this->validator->validate($dto); if (count($erros) > 0) { $mensagens = []; foreach ($erros as $erro) $mensagens[$erro->getPropertyPath()] = $erro->getMessage(); throw new BadRequestHttpException(json_encode(['erros' => $mensagens], JSON_UNESCAPED_UNICODE)); } return $dto; }
}
